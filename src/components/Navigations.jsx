        ...
        return _navigation.subNav ? (
          <div key={i} className="relative group py-[10px] menu">
            ...
            <div className="dropdown">
              <div className={`flex flex-col gap-[15px] bg-white shadow text-[14px] p-[20px] ${_navigation.name.toLowerCase()==='about us'?'min-w-[250px]':'min-w-[700px] grid grid-cols-[1fr_1fr_1fr] gap-[15px] p-[50px]'}`}>
              {_navigation.subNav.map((_subNav, idx) => (
                <Link to={_subNav.to} key={idx} className="text-dark-gray hover:font-[600]">{_subNav.name}</Link>
              ))}
              </div>
            </div>
          </div>
